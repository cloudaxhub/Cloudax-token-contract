...
import {
  CloudaxEcosystemVestingWallet,
  CloudaxEcosystemVestingWallet__factory,
...
describe('CloudaxTresauryVestingWallet', () => {
  ...
  beforeEach(async () => {
    ...
    const cloudaxMarketingVestingWalletFactory =
      (await ethers.getContractFactory(
        ...
        deployer
      )) as CloudaxEcosystemVestingWallet__factory;

      cloudaxEcosystemVestingWallet = await cloudaxMarketingVestingWalletFactory.deploy(
      john.address,
      deployer.address
    );

    // Wait for the contract to be mined
    await cloudaxEcosystemVestingWallet.deployed();
  });

  it('Should deploy the contract', async () => {
    expect(cloudaxEcosystemVestingWallet.address).to.not.equal(0);
  });
  ...
  describe('setBeneficiaryAddress', function () {
    it('should set the beneficiary address', async function () {
      ...
      await cloudaxEcosystemVestingWallet.setBeneficiaryAddress(newBeneficiary);
      const beneficiaryAddress = await cloudaxEcosystemVestingWallet.getBeneficiaryAddress();
  ...
  describe('initialize', function () {
    it('should initialize the vesting schedule', async function () {
      await cloudaxEcosystemVestingWallet.initialize(john.address);
      const beneficiaryAddress = await cloudaxEcosystemVestingWallet.getBeneficiaryAddress();
  ...
  describe('pause and unpause', function () {
    it('should pause and unpause the vesting release process', async function () {
      await cloudaxEcosystemVestingWallet.pause();
      expect(await cloudaxEcosystemVestingWallet.paused()).to.be.true;

      await cloudaxEcosystemVestingWallet.unpause();
  ...
  describe('_createVestingSchedule', function () {
    it('should create a new vesting schedule', async function () {
      ...
      await cloudaxEcosystemVestingWallet._createVestingSchedule(amount);
  ...
  describe('release', function () {
    it('should release the releasable amount when not paused and not reentrant', async function () {
      await cloudaxEcosystemVestingWallet.release();
  ...
  describe('withdraw', function () {
    it('should allow the owner to withdraw tokens when paused', async function () {
      ...
      await cloudaxEcosystemVestingWallet.withdraw(withdrawAmount);
    ...
    it('should revert if the withdraw amount exceeds the balance', async function () {
      ...
      await expect(cloudaxEcosystemVestingWallet.withdraw(withdrawAmount)).to.be.revertedWith('CloudrVesting: withdraw amount exceeds balance');
    });
  });
